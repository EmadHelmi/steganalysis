...
from torch.utils.data.dataloader import DataLoader, _MultiProcessingDataLoaderIter as _DataLoaderIter
    ...
    def __getitem__(self, idx):
        ...
        if idx % 2 == 0:
            ...
            stego_path = os.path.join(self.stego_dir,
                                      self.cover_list[cover_idx])
            images = np.array(Image.open(stego_path))
            # print("Hello", images.shape)
        samples = {'images': images[None, :, :, None], 'labels': labels}
        # print("Here", samples['images'].shape)
        if self.transform:
            samples = self.transform(samples)
            # print("Here4", samples['images'].shape)
...
class RandomBalancedSampler(Sampler):
    ...
    def __iter__(self):
        cover_perm = [x * 2 for x in torch.randperm(
            int(len(self.data_source) / 2)).long()]
        stego_perm = [x * 2 + 1 for x in torch.randperm(
                      int(len(self.data_source) / 2)).long()]
        # idx_list = torch.randperm(int(len(self.data_source) / 2)).long()
        # cover_perm = [x * 2 for x in idx_list]
        # stego_perm = [x * 2 + 1 for x in idx_list]
        return iter(it.__next__() for it in
...
class ToTensor(object):
    def __call__(self, samples):
        ...
        # print("tensor", images.shape)
        images = images.transpose((0, 3, 1, 2))
        # print("tensor2", images.shape)
...
class RandomRot(object):
    def __call__(self, samples):
        ...
        # print("rot", samples['images'].shape)
        rot = random.randint(0, 3)
        result = {'images': np.rot90(images, rot, axes=[1, 2]).copy(),
                  'labels': samples['labels']}
        # print("rot2", samples['images'].shape)
        return result
...
class RandomFlip(object):
    def __call__(self, samples):
        if random.random() < 0.5:
            images = samples['images']
            # print("flip", samples['images'].shape)
            result = {'images': np.flip(images, axis=2).copy(),
                      'labels': samples['labels']}
            # print("flip2", samples['images'].shape)
            return result
        else:
            return samples
