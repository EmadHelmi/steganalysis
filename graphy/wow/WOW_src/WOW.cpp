...
void printInfo(){
	std::cout << "This program embeds a payload using while minimizing 'WOW' steganographic distortion [1] to all greyscale 'PGM' images in the directory input-dir and saves the stego images into the output-dir." << std::endl << std::endl;
	std::cout << "[1] Designing Steganographic Distortion Using Directional Filters, Vojtech Holub and Jessica Fridrich, IEEE International Workshop on Information Forensics and Security, December 2012." << std::endl << std::endl;
	std::cout << "Author: Vojtech Holub, e-mail: vojtech_holub@yahoo.com" << std::endl << std::endl;
	std::cout << "usage: WOW -I input-dir -O output-dir -a payload [-v] [-p holder_p] [-h STC-height] \n\n";
}
...
int main(int argc, char** argv)
{
	try { 
        ...
        desc.add_options()
		    ...
		    ("holder_p,p",				po::value<float>(&p_holder)->default_value(-1),					"Holder norm parameter 'p' (default = -1)")
        ...
        if (verbose) {
            std::cout << "# WOW DISTORTION EMBEDDING SIMULATOR" << std::endl;
		...
		clock_t begin=clock();
		cost_model_config *config = new cost_model_config(payload, verbose, p_holder, wavelet, stc_constr_height, randSeed);
