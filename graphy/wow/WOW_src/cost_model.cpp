...
cost_model::cost_model(mat2D<int>* cover, cost_model_config* config) : base_cost_model(cover, (base_cost_model_config *)config)
{
	...
	mat2D<float> * xi_LH = mat2D<float>::Correlation_Same_basicFilters(mat2D<float>::AbsoluteValue(R_LH), mat2D<float>::AbsoluteValue(config->Tlpdf), mat2D<float>::AbsoluteValue(config->hpdf));
	mat2D<float> * xi_HL = mat2D<float>::Correlation_Same_basicFilters(mat2D<float>::AbsoluteValue(R_HL), mat2D<float>::AbsoluteValue(config->Thpdf), mat2D<float>::AbsoluteValue(config->lpdf));
	mat2D<float> * xi_HH = mat2D<float>::Correlation_Same_basicFilters(mat2D<float>::AbsoluteValue(R_HH), mat2D<float>::AbsoluteValue(config->Thpdf), mat2D<float>::AbsoluteValue(config->hpdf));
	...
	for (int r=0; r<cover->rows; r++)
	{
		...
		{
			float rho = pow( pow(xi_LH_no_padding->Read(r, c), float(config->p)) + pow(xi_HL_no_padding->Read(r, c), float(config->p)) + pow(xi_HH_no_padding->Read(r, c), float(config->p)) , (-1/float(config->p)));
