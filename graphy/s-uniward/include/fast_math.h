...
/* Based on the code from: http://jrfonseca.blogspot.com/2008/09/fast-sse2-pow-tables-or-polynomials.html */
...
inline __m128 __vrs4_expf(__m128 x) {

	__m128 const LOG2E = _mm_set1_ps( 1/log( 2.0f ) );

	return exp2f4( _mm_mul_ps(x, LOG2E) );
}

inline __m128 __vrs4_logf(__m128 x) {

	__m128 const LOG2 = _mm_set1_ps( log( 2.0f ) );

	return _mm_mul_ps(log2f4(x), LOG2);
}

inline __m128 exp_ps(__m128 x) {
	return __vrs4_expf(x);
}

inline __m128 log_ps(__m128 x) {
	return __vrs4_logf(x);
}
